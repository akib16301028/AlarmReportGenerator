...
# [All your existing functions remain unchanged except for the modifications mentioned above]

# Function to display footer using HTML
def display_footer(total_df, title="Summary"):
    html = f"""
...
if uploaded_alarm_file is not None and uploaded_offline_file is not None:
    try:
        ...
        # Display Total as Footer
        st.markdown("**Offline Report Summary**")
        total_summary = filtered_pivot_offline.select_dtypes(include=['number']).sum().to_dict()
        ...
        # Reorder columns to match pivot
        total_summary_df = total_summary_df[filtered_pivot_offline.columns]

        # Display as table
        st.table(total_summary_df)
        ...
        if not all(col in alarm_df.columns for col in alarm_required_columns):
            ...
            for alarm_name, (pivot, total_count) in alarm_data.items():
                ...
                # Display Total as Footer for each Alarm
                st.markdown("**Alarm Summary**")
                total_summary_alarm = pivot.select_dtypes(include=['number']).sum().to_dict()
                total_summary_alarm['Cluster'] = 'Total'
                total_summary_alarm['Zone'] = ''

                # Convert to DataFrame
                total_summary_alarm_df = pd.DataFrame([total_summary_alarm])

                # Reorder columns to match pivot
                total_summary_alarm_df = total_summary_alarm_df[pivot.columns]

                # Display as table
                st.table(total_summary_alarm_df)
